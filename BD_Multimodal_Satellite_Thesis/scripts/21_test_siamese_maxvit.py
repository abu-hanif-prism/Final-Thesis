"""Forward-pass tests for the multimodal Siamese MaxViT-inspired model."""
...
from src.models.siamese_maxvit import create_siamese_maxvit_model  # noqa: E402


def parse_args() -> argparse.Namespace:
    """Parse command-line arguments."""
    parser = argparse.ArgumentParser(description="Test multimodal Siamese MaxViT-inspired model.")
...
def main() -> None:
    ...
    for mode in modes:
        ...
        model = create_siamese_maxvit_model(output_mode=mode, window_size=args.window_size)
        test_model_on_batch(model, fake_batch, mode)

    if args.test_real_batch:
        test_real_batch(args)
    else:
...
def test_real_batch(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        train_loader, _, _ = create_train_val_test_dataloaders(
            ...
            num_workers=0,
            target_mode=args.output_mode if args.output_mode != "multitask" else "both",
            pin_memory=False,
        )
    ...
    model = create_siamese_maxvit_model(output_mode=args.output_mode, window_size=args.window_size)
    model.eval()
    with torch.no_grad():
        outputs = model(batch["image_t1"], batch["image_t2"], batch["tabular"])
    print(f"  real batch image_t1 shape: {tuple(batch['image_t1'].shape)}")
    print_output_shapes(outputs, args.output_mode, prefix="  real batch ")
