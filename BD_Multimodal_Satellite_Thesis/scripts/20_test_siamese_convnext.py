"""Forward-pass tests for the multimodal Siamese ConvNeXt model."""
...
from src.models.siamese_convnext import create_siamese_convnext_model  # noqa: E402


def parse_args() -> argparse.Namespace:
    """Parse command-line arguments."""
    parser = argparse.ArgumentParser(description="Test multimodal Siamese ConvNeXt model.")
...
def main() -> None:
    ...
    for mode in modes:
        ...
        model = create_siamese_convnext_model(output_mode=mode)
...
def test_real_batch(args: argparse.Namespace) -> None:
    ...
    model = create_siamese_convnext_model(output_mode=args.output_mode)
    model.eval()
    with torch.no_grad():
        outputs = model(batch["image_t1"], batch["image_t2"], batch["tabular"])
