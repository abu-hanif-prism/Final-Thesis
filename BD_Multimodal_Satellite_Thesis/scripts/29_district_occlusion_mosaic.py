"""Create district-level occlusion mosaics from selected patch predictions."""
...
from src.xai.occlusion_sensitivity import compute_occlusion_map
...
def parse_args() -> argparse.Namespace:
    """Parse CLI arguments."""
    parser = argparse.ArgumentParser(description="Build district-level occlusion XAI mosaics.")
    parser.add_argument("--model_name", default="maxvit", choices=["cnn", "convnext", "swin", "maxvit"])
    parser.add_argument("--experiment_name", default="maxvit_regression_stable")
    ...
    parser.add_argument("--target_image", default="t2", choices=["t1", "t2", "both"])
    parser.add_argument("--output_dir", default="outputs/xai/district_occlusion")
...
def read_csv_rows(path: str | Path) -> list[dict[str, str]]:
    """Read CSV rows with only the standard library."""
    csv_path = Path(path)
...
def sort_value(row: dict[str, Any], sort_by: str) -> float:
    """Return sortable score, placing missing values at the bottom."""
...
def selected_target_images(target_image: str) -> list[str]:
    """Return target image list from CLI value."""
    if target_image == "both":
        return ["t1", "t2"]
    return [target_image]
...
def resolve_run_dir(base_dir: Path) -> Path:
    """Avoid overwriting previous district occlusion outputs."""
    expected_names = {
        "selected_patches.csv",
        "district_occlusion_mosaic_t1.png",
        "district_occlusion_mosaic_t2.png",
        "district_prediction_selected_map.png",
        "district_occlusion_report.md",
    }
...
def save_patch_heatmap(heatmap: np.ndarray, path: Path, title: str) -> None:
    """Save one patch-level heatmap with matplotlib."""
...
def save_patch_overlay(rgb: np.ndarray, heatmap: np.ndarray, path: Path, title: str) -> None:
    """Save one patch-level RGB/heatmap overlay with matplotlib."""
    import matplotlib.pyplot as plt
...
def stitch_heatmaps(selected_rows: list[dict[str, Any]], target_image: str) -> tuple[np.ndarray, tuple[int, int, int, int]]:
    """Stitch selected 128x128 heatmaps into a district coordinate mosaic."""
    x_values = [int(row["x"]) for row in selected_rows if f"{target_image}_heatmap_array" in row]
    y_values = [int(row["y"]) for row in selected_rows if f"{target_image}_heatmap_array" in row]
    if not x_values or not y_values:
        raise ValueError(f"No heatmaps available for {target_image}.")
    ...
    for row in selected_rows:
        key = f"{target_image}_heatmap_array"
        if key not in row:
            continue
...
def save_mosaic_png(mosaic: np.ndarray, extent: tuple[int, int, int, int], path: Path, title: str) -> None:
    """Save a district-level heatmap mosaic."""
    import matplotlib.pyplot as plt
    ...
    ax.set_title(title)
    fig.colorbar(image, ax=ax, label="occlusion sensitivity")
    fig.tight_layout()
    fig.savefig(path, dpi=300)
    plt.close(fig)
...
def write_report(
    ...
    """Write district occlusion Markdown report."""
    ...
    lines = [
        "# District Occlusion Mosaic Report",
        "",
        ...
        f"- target image(s): `{', '.join(target_images)}`",
        f"- occlusion patch_size: {args.patch_size}",
        f"- occlusion stride: {args.stride}",
    ...
    for mosaic_path in mosaic_paths:
        lines.append(f"- district occlusion mosaic: `{mosaic_path}`")
    lines.extend(
        [
            "",
            ...
            "- Occlusion sensitivity was computed on selected 128x128 patches from the prediction CSV.",
            "- The patch-level heatmaps were stitched back into a district-level coordinate mosaic using x/y values parsed from patch_id.",
            "- Brighter regions indicate patches or sub-patch areas where masking changed the model prediction more.",
            "- This is model sensitivity, not confirmed physical causality.",
            "",
        ]
    )
...
def main() -> None:
    """Run selected patch occlusion and stitch results into district mosaics."""
    args = parse_args()
    prediction_csv = Path(args.prediction_csv) if args.prediction_csv else default_prediction_csv(args.experiment_name)
    index_path = Path(args.index_path)
    target_images = selected_target_images(args.target_image)
    ...
    patches_dir = run_dir / "patch_heatmaps"
    ...
    for index, row in enumerate(selected_rows, start=1):
        ...
        print(f"Occlusion {index}/{len(selected_rows)}: {patch_id}", flush=True)
        sample = load_npz_sample(row["npz_path"])
        batch = prepare_xai_batch(sample, device=device)
        sample_np = {
            "t1": np.asarray(sample["image_t1"]),
            "t2": np.asarray(sample["image_t2"]),
        }

        for target_image in target_images:
            heatmap, full_prediction = compute_occlusion_map(
                model,
                batch["image_t1"],
                batch["image_t2"],
                batch["tabular"],
            ...
            row[f"{target_image}_heatmap_array"] = heatmap_np
            row[f"{target_image}_full_prediction"] = full_prediction
            row[f"{target_image}_mean_importance"] = float(np.mean(heatmap_np))
            row[f"{target_image}_max_importance"] = float(np.max(heatmap_np))

            patch_dir = patches_dir / safe_name(patch_id)
            heatmap_path = patch_dir / f"{target_image}_occlusion_heatmap.png"
            overlay_path = patch_dir / f"{target_image}_occlusion_overlay.png"
            save_patch_heatmap(heatmap_np, heatmap_path, f"{target_image} occlusion\n{patch_id}")
            rgb = normalize_image_for_display(sample_np[target_image])
            save_patch_overlay(rgb, heatmap_np, overlay_path, f"{target_image} overlay\n{patch_id}")
            row[f"{target_image}_heatmap_path"] = str(heatmap_path)
            row[f"{target_image}_overlay_path"] = str(overlay_path)

    selected_csv_rows = [
        {key: value for key, value in row.items() if not key.endswith("_heatmap_array")}
        for row in selected_rows
    ]
    ...
    prediction_map_path = run_dir / "district_prediction_selected_map.png"
    save_selected_prediction_map(selected_rows, prediction_map_path, f"Selected predicted change ratio\n{title_base}")

    mosaic_paths: list[Path] = []
    for target_image in target_images:
        mosaic, extent = stitch_heatmaps(selected_rows, target_image)
        mosaic_path = run_dir / f"district_occlusion_mosaic_{target_image}.png"
        save_mosaic_png(mosaic, extent, mosaic_path, f"{target_image} district occlusion mosaic\n{title_base}")
        mosaic_paths.append(mosaic_path)

    report_path = run_dir / "district_occlusion_report.md"
    write_report(report_path, args, run_dir, selected_rows, target_images, mosaic_paths, prediction_map_path)

    print("District occlusion mosaic created.", flush=True)
